...
namespace gazebo_mecanum_plugins
{
  ...
  #endif
  // Update wheel velocities if new commands received
  UpdateWheelVelocities();
    ...
    // Apply acceleration limits or set target speed
    for(unsigned int i = 0; i < num_wheel_pairs_; ++i)
    {
      ...
      {
        joints_[2 * i + LEFT_REAR]->SetParam("vel", 0, desired_wheel_speed_[2 * i + LEFT_REAR]);
        joints_[2 * i + LEFT_FRONT]->SetParam("vel", 0, desired_wheel_speed_[2 * i + LEFT_FRONT]);
        joints_[2 * i + RIGHT_FRONT]->SetParam("vel", 0, desired_wheel_speed_[2 * i + RIGHT_FRONT]);
        joints_[2 * i + RIGHT_REAR]->SetParam("vel", 0, desired_wheel_speed_[2 * i + RIGHT_REAR]);
      }
      else
      {
        ...
        joints_[2 * i + LEFT_REAR]->SetParam("vel", 0, wheel_speed_instr_[2 * i + LEFT_REAR]);
        joints_[2 * i + LEFT_FRONT]->SetParam("vel", 0, wheel_speed_instr_[2 * i + LEFT_FRONT]);
        joints_[2 * i + RIGHT_FRONT]->SetParam("vel", 0, wheel_speed_instr_[2 * i + RIGHT_FRONT]);
        joints_[2 * i + RIGHT_REAR]->SetParam("vel", 0, wheel_speed_instr_[2 * i + RIGHT_REAR]);
      }
    }
